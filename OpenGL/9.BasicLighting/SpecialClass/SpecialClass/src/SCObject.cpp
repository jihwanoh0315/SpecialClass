...
Object::Object()
{
    ...
    // Generate vertex normal buffer
    glGenBuffers(1, &m_vnBuffer);
...
void Object::SetModel(Model* model_)
{
    ...
    ***********************************************************/
    ...
    glBindBuffer(GL_ARRAY_BUFFER, m_vnBuffer);
    glBufferData(GL_ARRAY_BUFFER, m_model->m_vertexNormal.size() * sizeof(glm::vec3),
        &m_model->m_vertexNormal[0], GL_STATIC_DRAW);
    ...
    glEnableVertexAttribArray(1); // Set attribue same with layout in the shader
...
void Object::Update(Shader& shader_, Camera& camera_)
{
    ...
    shader_.SetVec3("objectColor", m_color);
